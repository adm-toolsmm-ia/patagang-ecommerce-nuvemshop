# ✅ Checklist de Validação - Search Overlay

**Última atualização:** 2026-02-13 (Brasília)  
**Versão a validar:** conferir em `ftp-deploy/LAST_DEPLOY_VERSION.txt` (campo `VERSION_ID`). No navegador: F12 → Console → mensagem "VERSÃO DO DEPLOY".

**Data Validação:** ___/___/______  
...
- [ ] Versão atual visível no console da loja (igual a `VERSION_ID` em LAST_DEPLOY_VERSION.txt)
...
Listar backups: `node list-backups.js` (em ftp-deploy). Restaurar: `node rollback-incremental.js [TIMESTAMP]`. Ver ftp-deploy/README.md.

---

**Versão validada:** conferir em `ftp-deploy/LAST_DEPLOY_VERSION.txt`.
