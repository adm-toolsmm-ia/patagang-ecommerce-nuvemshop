# 🔍 Search Overlay – Deploy e referência técnica

**Última atualização:** 2026-02-13 (Brasília)  
**Versão em produção:** ver `ftp-deploy/LAST_DEPLOY_VERSION.txt`

**Estado atual do tema:** As edições descritas abaixo para `header-patagang.tpl` (trigger do overlay, include de header-search-overlay.tpl, JS do overlay) referem-se a um plano/versão em que o overlay seria integrado ao header. No tema atual, o botão de busca abre o modal `#nav-search` com `header-search.tpl` (campo de busca e filtros rápidos); `header-search-overlay.tpl` não é incluído no layout. Use este documento para entender a feature e o que seria necessário para ativá-la ou alterá-la; ao editar o header, consulte o arquivo atual no repositório.
...
```
✏️ Status (plano/versão descrita): EDITADO para integrar overlay
📍 Caminho: theme-deploy-corrigido/snipplets/header/header-patagang.tpl
🔄 Mudanças descritas (não aplicadas no tema atual):
...
No tema atual o header usa botão com js-modal-open + data-toggle="#nav-search" e modal com header-search.tpl.
```
...
### Imediatos (Hoje)
1. ✅ **Verificar deploy**: Confirmar no console da loja a versão indicada em `ftp-deploy/LAST_DEPLOY_VERSION.txt`
...
**Rollback (se necessário):** Listar backups: `node list-backups.js` ou `ls ../backups/incremental/`. Restaurar: `node rollback-incremental.js [TIMESTAMP]`. Ver `README.md`.
...
**Versão em produção:** ver `LAST_DEPLOY_VERSION.txt`. Próximos passos: limpar cache Nuvemshop e validar com [validation-checklist.md](validation-checklist.md).
