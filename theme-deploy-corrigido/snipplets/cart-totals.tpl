...
{% if not cart_page %}
  {# PATAGANG - Cart Summary #}
  <div class="pg-cart-summary js-visible-on-cart-filled" {% if cart.items_count == 0 %}style="display:none;"{% endif %}>

    {# Subtotal #}
    <div class="pg-cart-summary__row">
      <span class="pg-cart-summary__label">Subtotal</span>
      <span class="js-ajax-cart-total js-cart-subtotal pg-cart-summary__value" data-priceraw="{{ cart.subtotal }}" data-component="cart.subtotal" data-component-value="{{ cart.subtotal }}">{{ cart.subtotal | money }}</span>
    </div>

    {# Shipping Calculator - inline no modal #}
    {% if settings.shipping_calculator_cart_page and store.has_shipping %}
      <div id="cart-shipping-container" class="pg-cart-summary__shipping js-shipping-calculator-container" data-shipping-url="{{ store.shipping_calculator_url }}">
    ...
    {% else %}
      {# Fallback: link para página do carrinho #}
      <div class="pg-cart-summary__row">
        <span class="pg-cart-summary__label">Frete</span>
        <a href="{{ store.cart_url }}" class="pg-cart-summary__link">Ver na página do carrinho</a>
      </div>
    {% endif %}
...
{% endif %}
    ...
    {% endif %}
      {% if show_cart_fulfillment and cart_page %}
    ...
    {% endif %}
              ...
              {% else %}
              {# Ajax cart / modal: mantemos apenas o total oculto para compatibilidade com JS legado #}
              <div class='total-price hidden'>
                {{ "Total" | translate }}: {{ cart.total | money }}
              </div>
              {% endif %}
              ...
              <div class="js-visible-on-cart-filled {% if not cart_page %}container-fluid{% endif %}" {% if cart.items_count == 0 %}style="display:none;"{% endif %}>
                ...
                {% if cart_page %}
                  {% if cart.checkout_enabled %}
                    <input id="go-to-checkout" class="btn btn-primary btn-block mb-3" type="submit" name="go_to_checkout" value="{{ 'FINALIZAR COMPRA' | translate }}" data-component="cart.checkout-button"/>
                  {% else %}
                ...
                {# Cart panel continue buying link - Only on cart page #}

                {% if settings.continue_buying and cart_page %}
                  <div class="row mb-2">
                    <div class="text-center w-100">
                      <a href="{{ store.products_url }}" class="btn btn-link">{{ 'Ver más productos' | translate }}</a>
                    </div>
                  </div>
                {% endif %}
              </div>
