...
<div class="js-item-product {% if slide_item %}js-item-slide swiper-slide{% else %}col{% if columns == 2 %}-6 col-md-3{% else %}-12 col-md-4{% endif %}{% endif %} item item-product{% if not product.display_price %} no-price{% endif %} {% if reduced_item %}item-product-reduced{% endif %}" data-product-type="list" data-product-id="{{ product.id }}" data-store="product-item-{{ product.id }}" data-component="product-list-item" data-component-value="{{ product.id }}">
    ...
    {% endif %}
        ...
        {# PATAGANG: Botão especial para produtos em desenvolvimento #}
        {% if is_development_product and not reduced_item %}
            <div class="item-actions mt-2">
                <a href="{{ product_url_with_selected_variant }}" class="btn btn-primary btn-small px-4 mb-1 mx-auto btn-development" title="Seja o primeiro a conhecer {{ product.name }}">
        ...
        {% elseif product.available and product.display_price and not reduced_item %}
            ...
            <div class="item-actions mt-2">
                <a href="{{ product_url_with_selected_variant }}" class="btn btn-primary btn-small px-4 mb-1 mx-auto" title="{{ texts[state] }}">
                    {{ texts[state] }}
                </a>
            </div>
        {% endif %}
